import { generateSlug, extractCities, generateContent, generateMetaDescription, estimateRouteInfo } from '@/lib/routeUtils';
...
// All route titles from the user's list
const routeTitles = [
...
// Generate route data for all titles
export const routes: RouteData[] = routeTitles.map((title) => {
  try {
    const slug = generateSlug(title);
    const { from, to } = extractCities(title);
    const content = generateContent(from, to, title);
    const metaDescription = generateMetaDescription(from, to, title);
    const routeInfo = estimateRouteInfo(from, to);
    ...
    return {
      ...
      from,
      to,
      description: `Book reliable taxi service from ${from} to ${to} with RM Cabs. Professional drivers, safe vehicles, and best prices.`,
      content,
      metaDescription,
      ...routeInfo,
    };
  } catch (error) {
    console.error(`Error generating route for "${title}":`, error);
    // Return a fallback route
    return {
      title,
      slug: generateSlug(title),
      from: 'Bangalore',
      to: title.replace(/Taxi Service|Cabs From/gi, '').trim(),
      description: `Book reliable taxi service with RM Cabs. Professional drivers, safe vehicles, and best prices.`,
      content: `Book premium taxi service with RM Cabs. Professional drivers, GPS-tracked vehicles, and 24/7 customer support.`,
      metaDescription: `Book taxi service with RM Cabs. Professional drivers, GPS tracking, 24/7 support. Call +91 73490 91759.`,
    };
  }
});

// Create a map for quick lookup by slug
export const routesBySlug = new Map<string, RouteData>();
routes.forEach((route) => {
  // Handle duplicate slugs by appending a number
  let finalSlug = route.slug;
  let counter = 1;
  while (routesBySlug.has(finalSlug)) {
    finalSlug = `${route.slug}-${counter}`;
    counter++;
  }
